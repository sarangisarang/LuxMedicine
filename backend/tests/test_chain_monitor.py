...
async def forged(session: AsyncSession, seq: int | None = None):
    ...
    """
    if seq is None:
        seq = (await session.execute(text("SELECT MAX(seq) FROM audit_log"))).scalar_one()
...
async def test_a_successful_verification_records_a_checkpoint(session):
    ...
    result = await verify_and_checkpoint(session, verified_by="test")

    assert result.intact
    assert result.checkpoint_id is not None
    assert result.verified_through_seq == seq

    stored = await latest_checkpoint(session)
...
async def test_a_break_records_no_checkpoint(session):
    ...
    await append_one(session)
    await verify_and_checkpoint(session, verified_by="test")
...
async def test_a_checkpoint_turns_detection_into_a_time_bound(session):
    ...
    """
    await append_one(session)
    clean = await verify_and_checkpoint(session, verified_by="test")
    assert clean.intact

    async with forged(session):
        broken = await verify_and_checkpoint(session, verified_by="test")
